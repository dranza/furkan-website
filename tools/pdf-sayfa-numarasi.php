...
$pageTitle = 'PDF Sayfa Numarası Ekle';
$pageDesc  = 'PDF’e sayfa numarası ekle. Konum, başlangıç numarası ve biçim ayarları.';

Tool::ensure('pdf_pagenum', 'PDF Sayfa Numarası');
$tool = Tool::get('pdf_pagenum') ?? ['status'=>'active','meta'=>[]];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    Tool::recordUse('pdf_pagenum', 1, $files);
...
if ($toolStatus !== 'active') {
  ...
  render('tools_pdf_pagenum', compact('pageTitle','pageDesc','error'));
  return;
}

render('tools_pdf_pagenum', compact('pageTitle','pageDesc','toolStatus'));
