...
$pageTitle = 'Word → PDF';
$pageDesc  = 'DOCX belgelerini tarayıcı içinde PDF olarak dışa aktar. Dosyalar sunucuya yüklenmez.';

Tool::ensure('word_pdf', 'Word → PDF');
$tool = Tool::get('word_pdf') ?? ['status'=>'active','meta'=>[]];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    Tool::recordUse('word_pdf', 1, $files);
...
if ($toolStatus !== 'active') {
  ...
  render('tools_word_pdf', compact('pageTitle','pageDesc','error'));
  return;
}

render('tools_word_pdf', compact('pageTitle','pageDesc','toolStatus','toolMessage'));
