...
$pageTitle = 'PDF Sayfa Sil';
$pageDesc  = 'PDF içinden istemediğin sayfaları kaldır. Aralık/tekli seçim destekler. Tarayıcı içinde çalışır.';

Tool::ensure('pdf_delete_pages', 'PDF Sayfa Sil');
$tool = Tool::get('pdf_delete_pages') ?? ['status'=>'active','meta'=>[]];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    $tNow = Tool::get('pdf_delete_pages');
    ...
    Tool::recordUse('pdf_delete_pages', 1, $pages);
...
if ($toolStatus !== 'active') {
  ...
  render('tools_pdf_delete_pages', compact('pageTitle','pageDesc','error'));
  return;
}

render('tools_pdf_delete_pages', compact('pageTitle','pageDesc','toolStatus','toolMessage'));
