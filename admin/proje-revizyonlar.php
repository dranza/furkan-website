...
$project = $projectId ? Projects::adminGet($projectId) : null;
if (!$project) { echo "<div class='alert alert-danger'>Kayıt bulunamadı.</div>"; require __DIR__.'/_layout_bottom.php'; exit; }

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  CSRF::checkOrExit($_POST['_csrf'] ?? null);
  $revId = (int)($_POST['rev_id'] ?? 0);
  $rev = Revisions::projectGet($revId);
  if ($rev && (int)$rev['project_id'] === $projectId) {
    Projects::adminSave([
      'id' => $projectId,
      'title' => $rev['title'],
      'slug' => $rev['slug'],
      'summary' => $rev['summary'],
      'details' => $rev['details'],
      'technologies' => $rev['technologies'],
      'cover_image' => $rev['cover_image'],
      'meta_title' => $rev['meta_title'],
      'meta_desc' => $rev['meta_desc'],
      'featured' => (int)$rev['featured'],
      'status' => $rev['status'],
      'published_at' => $rev['published_at'],
    ]);
    redirect(base_url('admin/proje-duzenle.php?id='.$projectId.'&restored=1'));
  }
}

$list = Revisions::projectList($projectId);
?>
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h1 class="h4 fw-bold mb-1">Revizyonlar</h1>
    <div class="text-muted"><?= e($project['title']) ?></div>
  </div>
  <a class="btn btn-outline-light btn-sm" href="<?= e(base_url('admin/proje-duzenle.php?id='.$projectId)) ?>"><i class="bi bi-arrow-left me-1"></i>Geri</a>
</div>
