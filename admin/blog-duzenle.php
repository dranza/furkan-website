...
$item = $id ? Blog::adminGet($id) : null;

$ok = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if ($item && !empty($item['id'])) { try { Revisions::saveBlog($item); } catch (Throwable $t) {} }

  CSRF::checkOrExit($_POST['_csrf'] ?? null);

  $cover = trim((string)($_POST['cover_image'] ?? '')) ?: ($item['cover_image'] ?? '');
  $up = handle_upload('cover', 'blog');
  if ($up) $cover = $up;

  $savedId = Blog::adminSave([
    'id' => $_POST['id'] ?: null,
  ...
  $ok = 'Kaydedildi.';
  $id = $savedId;
  $item = Blog::adminGet($id);

  // Cache purge: only related pages
  $paths = ['/'];
  if (!empty($item['slug'])) $paths[] = '/blog/'.$item['slug'];
  $paths[] = '/blog';
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div>
    <h1 class="h4 fw-bold mb-1"><?= $id ? 'Blog Düzenle' : 'Yeni Blog Yazısı' ?></h1>
    <div class="text-muted">Kategori + etiket + SEO + medya kütüphanesi</div>
  </div>
  <div class="d-flex gap-2">
    <a class="btn btn-outline-light btn-sm" href="<?= e(base_url('admin/blog.php')) ?>"><i class="bi bi-arrow-left me-1"></i>Geri</a>
    <?php if (!empty($item['slug']) && ($item['status'] ?? '') === 'published'): ?>
      <a class="btn btn-outline-light btn-sm" target="_blank" href="<?= e(base_url('blog/'.$item['slug'])) ?>"><i class="bi bi-box-arrow-up-right me-1"></i>Görüntüle</a>
    <?php endif; ?>
  </div>
</div>
...
<form method="post" enctype="multipart/form-data" class="card p-3">
  ...
  <div class="row g-3">
    ...
    <div class="col-md-4">
      ...
      <input class="form-control" name="category" value="<?= e($item['category'] ?? '') ?>" placeholder="örn: Entegrasyon">
    </div>
    <div class="col-md-5">
      <label class="form-label">Etiketler (virgülle)</label>
      <input class="form-control" name="tags" value="<?= e($item['tags'] ?? '') ?>" placeholder="hl7, sql, active directory">
    </div>
    <div class="col-md-3">
    ...
    <div class="form-text">Gelecek tarih girersen içerik otomatik olarak o tarihte yayına alınır.</div>
    </div>
    ...
    <div class="col-md-9">
      ...
      <div class="row g-2 align-items-end">
        <div class="col-lg-6">
          ...
          <div class="form-text">Yükle veya aşağıdan medya seç</div>
        </div>
        <div class="col-lg-6">
  ...
  <span>İçerik</span>
  <button type="button" class="btn btn-sm btn-outline-light" onclick="insertBlogTemplate()"><i class="bi bi-journal-code me-1"></i>Teknik Yazı Şablonu</button>
</label>
      <textarea id="editor_blog" class="form-control" name="content" rows="14"><?= e($item['content'] ?? '') ?></textarea>
    </div>
  </div>

  <div class="mt-3 d-flex gap-2 flex-wrap">
    <button class="btn btn-primary"><i class="bi bi-check2 me-1"></i>Kaydet</button>
    <a class="btn btn-outline-light" href="<?= e(base_url('admin/blog.php')) ?>">Listeye dön</a>
    <a class="btn btn-outline-light" href="<?= e(base_url('admin/blog-revizyonlar.php?id='.$id)) ?>"><i class="bi bi-clock-history me-1"></i>Revizyonlar</a>
...
<script>
  ClassicEditor.create(document.querySelector('#editor_blog')).then(ed=>{window._editorBlog=ed;}).catch(console.error);
function insertBlogTemplate(){
  if(!window._editorBlog){return;}
  const html = `<h2>Özet</h2><p>Kısa özet.</p><h2>Problem</h2><p>Senaryo.</p><h2>Çözüm Adımları</h2><ol><li>Adım 1</li><li>Adım 2</li></ol><h2>Kod</h2><pre><code class=\"language-php\">// örnek kod</code></pre><h2>Sonuç</h2><p>Özetle…</p>`;
  window._editorBlog.setData(html);
}
