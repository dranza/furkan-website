<div class="container py-5">
  <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    <div>
      <h1 class="fw-bold mb-1">PDF Sayfa Sil</h1>
      <div class="text-muted">PDF içinden istemediğin sayfaları kaldır. Dosyalar sunucuya yüklenmez.</div>
    </div>
  ...
  <div class="row g-4">
    <div class="col-lg-5">
      <div class="card shadow-sm">
        <div class="card-body">
          ...
          <div class="mt-3">
            <label class="form-label fw-semibold">Silinecek sayfalar</label>
            <input id="pages" class="form-control" placeholder="örn: 1,3,5-7" />
            <div class="form-text">Virgülle ayır. Aralık kullanabilirsin: 5-7</div>
          </div>

          <div class="mt-3">
            <label class="form-label fw-semibold">Çıktı dosya adı</label>
            <input id="outName" class="form-control" value="duzenlenmis.pdf" />
          </div>

          <button id="btnRun" class="btn btn-primary w-100 mt-3" disabled>Sil ve indir</button>
    ...
    <div class="col-lg-7">
      <div class="card shadow-sm">
        <div class="card-body">
          <h5 class="fw-semibold mb-1">İpucu</h5>
          <div class="text-muted">Örnekler: <span class="badge text-bg-light border">1,3,5</span> <span class="badge text-bg-light border">2-4</span> <span class="badge text-bg-light border">1,3-5,9</span></div>
          <hr />
...
(function(){
  ...
  function parseSpec(spec, maxPages){
    const s=(spec||'').trim();
    if (!s) return [];
  ...
  function setFile(f){
    ...
    status.textContent='Hazır. Silinecek sayfaları yazıp butona bas.';
  }
  ...
  btnClear.addEventListener('click',()=>{ file=null; pdfInput.value=''; pages.value=''; status.textContent='Hazır.'; setFile(null); setEnabled(); });
  ...
  async function record(pagesCount){
    try{ await fetch('<?= e(base_url('araclar/pdf-sayfa-sil')) ?>', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({pages: pagesCount})}); }catch(e){}
  }
  ...
  btnRun.addEventListener('click', async ()=>{
    ...
    try{
      ...
      const del = parseSpec(pages.value, total);
      if (!del.length) throw new Error('Silinecek sayfaları gir.');
      if (del.length===total) throw new Error('Tüm sayfaları silemezsin.');

      // pdf-lib deletes by 0-based index; delete from end
      const idx = del.map(n=>n-1).sort((a,b)=>b-a);
      idx.forEach(i=>doc.removePage(i));

      const outBytes=await doc.save();
      const blob=new Blob([outBytes], {type:'application/pdf'});
      const name=(outName.value||'duzenlenmis.pdf').trim() || 'duzenlenmis.pdf';
      download(blob, name.endsWith('.pdf')?name:(name+'.pdf'));

      status.textContent=`Tamamlandı. Silinen sayfa: ${del.join(', ')}`;
      record(total);
    } catch(err){
      console.error(err);
